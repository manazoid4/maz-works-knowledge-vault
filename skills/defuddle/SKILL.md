...
**Substrate note (v1.7+)**: Unlike `obsidian-markdown` / `obsidian-bases` / `json-canvas` (where we defer to kepano/obsidian-skills), this `defuddle` workflow comes from the vault's MIT-licensed upstream foundation; kepano's marketplace does not ship a defuddle skill. The underlying `defuddle-cli` is independent and lives at [github.com/kepano/defuddle](https://github.com/kepano/defuddle).
