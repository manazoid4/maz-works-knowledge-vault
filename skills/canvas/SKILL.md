...
**Substrate preference (v1.7+)**: This skill is a self-contained fallback. **Prefer `kepano/obsidian-skills`** as the authoritative substrate — its `json-canvas` skill is the canonical spec reference. If a separate `json-canvas` skill is available, use it for spec questions. Continue to use this `canvas` skill for Maz Works vault workflows such as positioning into `wiki/canvases/`, image integration, and zone layout. Install kepano with `claude plugin marketplace add kepano/obsidian-skills`.
